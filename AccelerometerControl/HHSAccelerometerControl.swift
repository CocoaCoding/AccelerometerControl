...
public class HHSAccelerometerControl
{
    ...
    func setValue(value:Double)
    {
        ...
        if iValue != self.gaugeValue
        {
           self.gaugeValue = iValue
            self.redrawControl()
        }
    }
    ...
    private func redrawControl()
    {
        ...
        {
            let precentage = Double(self.gaugeValue) * -1.0
        ...
        else if self.gaugeValue > 0
        {
            let precentage = Double(self.gaugeValue)
            let segCount = Double(self.segmentsColors.count) / 100.0 *  precentage
            let segmentCount = Int(round(segCount))
